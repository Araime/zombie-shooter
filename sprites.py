...
class Player(pg.sprite.Sprite):
    ...
    def shoot(self):
        ...
        if now - self.last_shot > WEAPONS[self.weapon]['rate']:
            ...
            for blt in range(WEAPONS[self.weapon]['bullet_count']):
                spread = uniform(-WEAPONS[self.weapon]['spread'], WEAPONS[self.weapon]['spread'])
                blt = Bullet(self.game, pos, dir.rotate(spread), WEAPONS[self.weapon]['damage'])
                blt.alert_mobs()
...
class Mob(pg.sprite.Sprite):
    ...
    def update(self):
        ...
        if target_dist.length_squared() < DETECT_RADIUS**2 or self.is_chasing:
            if random() < 0.002:
    ...
    def alert_mobs(self):
        for target in self.targets:
            if not target.is_chasing:
                ...
                if target_dist.length_squared() < DETECT_RADIUS ** 2.2:
